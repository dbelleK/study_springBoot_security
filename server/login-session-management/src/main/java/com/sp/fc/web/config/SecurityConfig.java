...
//                                .accessDeniedPage("/access-denied")
                ...
                .sessionManagement( //세션 아이디 고정되있는거 아님 -> 고정되어 있다면 도용문제발생
                        s->s
//                                .sessionCreationPolicy(p-> SessionCreationPolicy.S)
                        ...
                        .maximumSessions(2) //1 , true(막겠다) 이라면  두개 이상의 창에서 로그인이 둘다 안되고 한개의 창에서만 됨
                        .maxSessionsPreventsLogin(true) //1, fase 이면 두 개 이상이면 막지는 않고 꺼지겠다
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring()
                .antMatchers("/sessions", "/session/expire", "/session-expired") //웹 리소스로 등록 혹은 권한체크 안함
                .requestMatchers(
